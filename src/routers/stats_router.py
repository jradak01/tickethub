...
async def get_stats(request: Request):
    ...
    medium_priority_tickets = sum(
        1 for t in tickets if t.priority == TicketPriority.medium
    )
    ...
    max_user = (
        max(tickets_per_user, key=tickets_per_user.get) if tickets_per_user else None
    )
    ...
    return {
        ...
        "status_percent": {
            "open": (
                round(open_tickets / total_tickets * 100, 2) if total_tickets > 0 else 0
            ),
            "closed": (
                round(closed_tickets / total_tickets * 100, 2)
                if total_tickets > 0
                else 0
            ),
        },
        ...
        "tickets_per_user": {
            ...
            "top_user": {"username": max_user, "tickets": max_tickets},
        },
    }
...
def distribution_per_user(
    tickets_per_user: dict[str, int], max_bucket: int
) -> dict[str, int]:
    """Calculate distribution of tickets per user."""
