...
async def test_get_users_count_success():
    with patch("services.user_service.get_cache", return_value=None), patch("services.user_service.set_cache"):
        ...
        with patch("services.user_service.httpx.AsyncClient.get", return_value=mock_response):
...
async def test_get_users_count_called_with_correct_params():
    with patch("services.user_service.get_cache", return_value=None), patch("services.user_service.set_cache"):
        ...
        # Patching the httpx.AsyncClient.get method to return the mock response
        with patch("services.user_service.httpx.AsyncClient.get", return_value=mock_response) as mock_get:
...
async def test_get_users_count_http_status_error():
    with patch("services.user_service.get_cache", return_value=None), patch("services.user_service.set_cache"):
        ...
        with patch("services.user_service.httpx.AsyncClient.get", side_effect=raise_error):
            # Calling the get_users_count function and checking the result
            with pytest.raises(HTTPException) as e:
                await get_users_count()
