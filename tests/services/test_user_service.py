...
async def test_get_users_count_success():
    with patch("src.services.user_service.get_cache", return_value=None), patch(
        "src.services.user_service.set_cache"
    ):
        ...
        with patch(
            "src.services.user_service.httpx.AsyncClient.get",
    ...
    with patch("src.services.user_service.get_cache", return_value=None), patch(
        ...
        with patch(
            ...
            mock_get.assert_called_once_with(
                "https://dummyjson.com/users", params={"limit": 1}
            )
            assert result == 50
...
async def test_get_users_count_http_status_error():
    with patch("src.services.user_service.get_cache", return_value=None), patch(
        ...
        exc = HTTPStatusError(
            "Error", request=Request("GET", "url"), response=Response(500)
        )
        ...
        with patch(
            "src.services.user_service.httpx.AsyncClient.get", side_effect=raise_error
        ):
            # Calling the get_users_count function and checking the result
            with pytest.raises(HTTPException) as e:
                await get_users_count()
