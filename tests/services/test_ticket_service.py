...
async def test_get_tickets_success():
    ...
    with patch("src.services.ticket_service.get_cache", return_value=None), patch(
        "src.services.ticket_service.set_cache"
    ):
        ...
        mock_todos_response.json = lambda: {
            "total": 10,
            "todos": [{"id": 1, "todo": "Some Todo", "completed": False, "userId": 1}],
        }
...
async def test_get_tickets_unexpected_error():
    ...
    with patch("src.services.ticket_service.get_cache", return_value=None), patch(
        ...
        mock_todos_response.json = AsyncMock(
            return_value={"total": 10}
        )  # This won't be used due to the error

        # Patch the HTTP GET request to return the faulty mock response
        with patch(
            "src.services.ticket_service.httpx.AsyncClient.get",
            return_value=mock_todos_response,
        ):
...
async def test_get_ticket_by_id_not_found():
    ...
    with patch("src.services.ticket_service.get_cache", return_value=None), patch(
        ...
        with patch(
            "src.services.ticket_service.httpx.AsyncClient.get",
            return_value=mock_todo_response,
        ):
