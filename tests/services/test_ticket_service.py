...
from services.ticket_service import get_tickets, get_ticket_by_id
from fastapi import HTTPException
from models.ticket_model import Ticket, TicketStatus, TicketPriority, TicketWithRawResponse
import httpx
...
# Patch cache functions to avoid real Redis calls during the test
    with patch("services.ticket_service.get_cache", return_value=None), \
         patch("services.ticket_service.set_cache"):
        ...
        with patch("services.ticket_service.httpx.AsyncClient.get", 
...
async def test_get_tickets_unexpected_error():
    ...
    with patch("services.ticket_service.get_cache", return_value=None), \
        ...
        with patch("services.ticket_service.httpx.AsyncClient.get", return_value=mock_todos_response):
...
async def test_get_tickets_empty_response():
    ...
    with patch("services.ticket_service.get_cache", return_value=None), \
        ...
        with patch(
            "services.ticket_service.httpx.AsyncClient.get",
...
async def test_get_ticket_by_id_not_found():
    ...
    with patch("services.ticket_service.get_cache", return_value=None), \
        ...
        with patch("services.ticket_service.httpx.AsyncClient.get", return_value=mock_todo_response):
