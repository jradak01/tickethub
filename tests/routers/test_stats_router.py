...
def mock_get_users_count(mocker):
    ...
    mocker.patch(
        "src.routers.stats_router.get_users_count", mock
    )  # Patch the target function with the mock
...
def mock_get_tickets(mocker):
    mock = AsyncMock()  # Create an AsyncMock instance
    mocker.patch(
        "src.routers.stats_router.get_tickets", mock
    )  # Patch the target function with the mock
    return mock  # Return the mock for use in tests
...
async def test_get_stats(mock_get_tickets, mock_get_users_count):
    ...
    mock_get_tickets.return_value = {
        "tickets": [
            Ticket(
                id=1, title="Ticket 1", status="open", priority="low", assignee="user_1"
            ),
            Ticket(
                id=2,
                title="Ticket 2",
    ...
    assert (
        data["tickets_per_user"]["top_user"]["username"] == "user_1"
    )  # User with the highest number of tickets
    assert (
        data["tickets_per_user"]["top_user"]["tickets"] == 2
    )  # Number of tickets of top user
