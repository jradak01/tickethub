...
from services.ticket_service import get_tickets
from services.user_service import get_users_count

# Create a TestClient instance for testing FastAPI app endpoints
client = TestClient(app)

# Fixture to mock the async function 'get_users_count' in 'routers.stats'
@pytest.fixture
def mock_get_users_count(mocker):
    mock = AsyncMock()  # Create an AsyncMock instance
    mocker.patch('routers.stats_router.get_users_count', mock)  # Patch the target function with the mock
    return mock  # Return the mock for use in tests

# Fixture to mock the async function 'get_tickets' in 'routers.stats'
@pytest.fixture
def mock_get_tickets(mocker):
    mock = AsyncMock()  # Create an AsyncMock instance
    mocker.patch('routers.stats_router.get_tickets', mock)  # Patch the target function with the mock
    return mock  # Return the mock for use in tests
